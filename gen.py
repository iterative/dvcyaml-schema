...
class ParamKey(BaseModel):
    __root__: str = Field(..., desc="Parameter key (dot-separated).")
...
STAGE_VARS_DESC = """\
Sequence of variables for parametrization.

May include any mapping or a path to a params file (which may be a string or a \
mapping to params in the file).
...
METRICS_DESC = "Sequence of metrics of the stage written to JSON/TOML/YAML for \
evaluation."

PLOTS_DESC = """\
Sequence of plots of the stage for visualization.
...
class Stage(BaseModel):
    ...
    deps: Optional[Dependencies] = Field(
        None, description="Sequence of the dependencies for the stage."
    )
    params: Optional[Params] = Field(None, description=PARAMS_DESC)
    outs: Optional[Outs] = Field(
            None, description="Sequence of the outputs of the stage."
    )
...
FOREACH_DESC = """\
Iterable to loop through in foreach. Can be a parametrized string, sequence or \
a mapping.
